...
_CANCEL_WORDS = frozenset([
    "الغاء", "إلغاء", "الغي", "ألغي", "مش عايز", "مش عايزة", "cancel", "stop", "إلغي"
])
...
_RE_NAME_EN = re.compile(
    r"(?:my name is|i'm|i am|this is|call me)\s+([a-zA-Z]{2,}(?:\s+[a-zA-Z]{2,})?)",
    re.I,
)
...
class StateMachine:
    """
    Drives the multi-turn purchase collection flow.
    Returns (handled: bool, next_intent: str | None).
    When handled=True, the orchestrator must use the response set here
    ...
    def run(bundle: "SignalBundle", conv) -> bool:
        """
        Process the current bundle against the active purchase state.
        Returns True if the state machine fully handled this turn
        (the orchestrator should skip LLM generation).
        Sets bundle.response and bundle.intent accordingly.
        """
    ...
    def start_purchase_flow(bundle: "SignalBundle", conv) -> bool:
        """
        Called when user signals purchase intent (e.g. "ممكن اطلبه").
        Determines the first missing field and asks for it.
        Returns True (handled).
        """
        if not bundle.last_product_id and not bundle.last_product_snapshot:
            bundle.response = (
                "مش عارف تقصد أي منتج تحديداً 😊 ممكن تقولي اسم المنتج اللي عايزه؟"
                if bundle.language == "ar"
                else "I'm not sure which product you mean. Could you mention the product name?"
            )
            bundle.response_conf = 0.95
            return True

        return StateMachine._advance_from_idle(bundle, conv)

    @staticmethod
    def _advance_from_idle(bundle: "SignalBundle", conv) -> bool:
        if not bundle.user_name:
            StateMachine._set_state(bundle, conv, PurchaseState.AWAITING_NAME)
            bundle.response = (
                "تمام 😊 محتاج اسمك الكامل عشان أكمل الطلب."
                if bundle.language == "ar"
                else "Great! Could you share your full name to proceed with the order?"
            )
            bundle.response_conf = 0.95
        ...
        if not bundle.phone:
            ...
            bundle.response = (
                f"تمام يا {bundle.user_name} 😊 ممكن رقم هاتفك؟"
                if bundle.language == "ar"
                else f"Great {bundle.user_name}! Could you share your phone number?"
            )
            bundle.response_conf = 0.95
            bundle.intent = "purchase_intent"
            return True
        ...
        if not bundle.address:
            ...
            bundle.response = (
                "ممتاز 😊 محتاج عنوان التوصيل."
                if bundle.language == "ar"
                else "Perfect! Could you share your delivery address?"
            )
            bundle.response_conf = 0.95
            bundle.intent = "purchase_intent"
            return True
        ...
        if not bundle.quantity:
            ...
            bundle.response = (
                "كام قطعة تحب تطلب؟"
                if bundle.language == "ar"
                else "How many units would you like to order?"
            )
            bundle.response_conf = 0.95
            bundle.intent = "purchase_intent"
            return True
    ...
    def _collect_name(bundle: "SignalBundle", conv) -> bool:
        ...
        if name:
            ...
            bundle.response = (
                f"أهلاً {name} 😊 ممكن رقم هاتفك؟"
                if bundle.language == "ar"
                else f"Nice to meet you, {name}! Could you share your phone number?"
            )
            bundle.response_conf = 0.95
            return True

        bundle.response = (
            "معنديش اسمك لسه 😊 ممكن تقولي اسمك الكامل؟"
            if bundle.language == "ar"
            else "Could you please share your full name?"
        )
        bundle.response_conf = 0.95
        return True
    ...
    def _collect_phone(bundle: "SignalBundle", conv) -> bool:
        ...
        if phone:
            ...
            bundle.response = (
                "تمام 😊 دلوقتي محتاج عنوان التوصيل."
                if bundle.language == "ar"
                else "Got it! Now I need your delivery address."
            )
            bundle.response_conf = 0.95
            return True

        bundle.response = (
            "الرقم ده مش صح 😊 ممكن تبعتلي رقم مصري صحيح (مثال: 01012345678)؟"
            if bundle.language == "ar"
            else "That doesn't look like a valid Egyptian number. Please share a valid phone (e.g. 01012345678)."
        )
        bundle.response_conf = 0.95
        return True
    ...
    def _collect_address(bundle: "SignalBundle", conv) -> bool:
        ...
        if is_valid_address(candidate):
            ...
            bundle.response = (
                "تمام 😊 كام قطعة تحب تطلب؟"
                if bundle.language == "ar"
                else "Got it! How many units would you like to order?"
            )
            bundle.response_conf = 0.95
            return True

        bundle.response = (
            "ممكن تبعتلي عنوان تفصيلي أكتر؟ (مثال: القاهرة - مدينة نصر - شارع عباس)"
            if bundle.language == "ar"
            else "Could you share a more detailed address? (e.g. Cairo, Nasr City, Abbas St.)"
        )
        bundle.response_conf = 0.95
        return True
    ...
    def _collect_quantity(bundle: "SignalBundle", conv) -> bool:
        ...
        bundle.response = (
            "ممكن تبعتلي الكمية كرقم؟ (مثال: 1 أو 2)"
            if bundle.language == "ar"
            else "Could you share the quantity as a number? (e.g. 1 or 2)"
        )
        bundle.response_conf = 0.95
        return True
    ...
    def _handle_cancel(bundle: "SignalBundle", conv) -> bool:
        ...
        bundle.response = (
            "تم إلغاء الطلب 😊 أقدر أساعدك في أي حاجة تانية؟"
            if bundle.language == "ar"
            else "Order cancelled. How else can I help you?"
        )
        bundle.response_conf = 0.95
        return True
